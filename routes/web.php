...
Route::get('/remove-special-character', function () {

    User::whereNull('old_firstname')
        ->whereNull('old_lastname')
        ->whereNull('old_name')
        ->chunkById(100, function ($users) {
            foreach ($users as $user) {

                // 1) Keep original values only once
                $user->old_firstname = $user->firstname;
                $user->old_lastname  = $user->lastname;
                $user->old_name      = $user->name;

                // 2) Clean values (remove special characters)
                $user->firstname = clean_name($user->firstname);
                $user->lastname  = clean_name($user->lastname);
    ...
    return "Special characters removed from user names.";
});

Route::get('/revert-special-character', function () {

    User::where(function ($q) {
            $q->whereNotNull('old_firstname')
              ->orWhereNotNull('old_lastname')
              ->orWhereNotNull('old_name');
        })
        ->chunkById(100, function ($users) {
            foreach ($users as $user) {

                // Restore only if backup exists
                if (!is_null($user->old_firstname)) {
                    $user->firstname = $user->old_firstname;
                }

                if (!is_null($user->old_lastname)) {
                    $user->lastname = $user->old_lastname;
                }

                if (!is_null($user->old_name)) {
                    $user->name = $user->old_name;
                }

                // Clear backups so this revert only happens once
                $user->old_firstname = null;
                $user->old_lastname  = null;
                $user->old_name      = null;
