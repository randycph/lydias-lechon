    ...
    public function checkout()
{
    ...
    if (Auth::check()) {
        ...
        foreach ($sessionCarts as $key => $cart) {
            ...
            // Check product types from session
            if (isset($cart['product']['category_id']) && $cart['product']['category_id'] == 1) {
                $haslechon = true;
            }
            if (isset($cart['product']['slug']) && $cart['product']['slug'] == 'lechon-baka') {
                $hasbaka = true;
            }
            if (isset($cart['product']['is_misc']) && $cart['product']['is_misc'] == 1) {
                $hasMisc = true;
            }
            if (isset($cart['product_id']) && $cart['product_id'] === 165) {
                $hasCochinillo = true;
            }
        }
    ...
    // Get eligible regular coupons (non-auto)
    $eligibleCoupons = Coupon::query()
    ...
    // Check cart eligibility for auto coupons
    ...
    if ($carts->count() > 0) {
        ...
        if (!$cartHasExcludedCategory) {
            ...
            if ($coupon->customer_limit !== null && $customerUsed >= $coupon->customer_limit) continue;
                ...
                // Check purchase combination conditions
                $shouldSkip = false;
                ...
                $eligibleAutoCoupons->push($couponData);
            }
        }
    }
    ...
    // Merge all coupons for display in modal
    $allCoupons = collect($eligibleCoupons->values()->toArray())
