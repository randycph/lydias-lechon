<div class="mt-5 px-4">
    ...
    <form class="mx-auto mt-3" id="contact-form" action="{{ route('contact-us') }}" method="POST">
        ...
        <div class="mb-5">
            ...
            <input value="{{ old('contact') }}" type="tel" id="phone" name="contact"
                class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                required />
            @error('contact')
                <div class="text-red-500 mt-1">{{ $message }}</div>
        ...
        <button
            class="g-recaptcha text-white bg-primary mt-4 hover:bg-primary-dark focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg w-full sm:w-auto px-5 py-3 text-center"
            data-sitekey="{{ config('services.recaptcha.site') }}" data-callback="onSubmit">
            Submit
        </button>
