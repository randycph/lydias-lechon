...
<div class="relative container">
    <div class="swiper swiper-blogs relative mt-10">
        <div class="swiper-wrapper">
            ...
            <a href="{{ route('article', ['category' => $blog?->category->slug, 'slug' => $blog?->slug]) }}" class="swiper-slide flex flex-col items-center gap-2 p-4 group">
                <img src="{{ $blog->thumbnail_url }}" alt="{{ $blog->name }}" class="default w-full rounded-lg">
                <div class="text-gray-600 text-sm lg:text-base mt-2">{{ $blog?->created_at->format('F d, Y') }}</div>
                <div class="font-semibold text-base lg:text-lg group-hover:text-gray-700">{{ $blog->name }}</div>
            </a>
            @endforeach
        </div>
    </div>
