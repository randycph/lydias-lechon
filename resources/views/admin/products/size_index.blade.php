    ...
    <div class="container pd-x-0">
        <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
            <div>
                ...
                <h4 class="mg-b-0 tx-spacing--1">Manage Product Sizes</h4>
            </div>
        </div>
        ...
        <div class="row row-sm">
                ...
                <div class="filter-buttons">
                    <div class="d-md-flex bd-highlight">
                        <div class="bd-highlight mg-r-10 mg-t-10">
                            ...
                            @if (auth()->user()->has_access_to_route('sizes.multiple.change.status') || auth()->user()->has_access_to_route('sizes.multiple.delete'))
                                <div class="list-search d-inline">
                                    <div class="dropdown d-inline mg-r-10">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            @if (auth()->user()->has_access_to_route('size.multiple.change.status'))
                                            ...
                                            @endif
                                            @if (auth()->user()->has_access_to_route('size.multiple.delete'))
                                                <a class="dropdown-item tx-danger" href="javascript:void(0)" onclick="delete_size()">{{__('common.delete')}}</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="mg-t-10">
                            @if (auth()->user()->has_access_to_route('sizes.create'))
                                <a class="btn btn-primary btn-sm mg-b-20" href="{{route('sizes.create')}}">Create Size</a>
                            @endif
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table mg-b-0 table-light table-hover">
                            ...
                            @forelse($sizes as $size)
                                <tr id="row{{$size->id}}">
                                    <th>
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input cb" id="cb{{ $size->id }}">
                                            <label class="custom-control-label" for="cb{{ $size->id }}"></label>
                                        </div>
                                    </th>
                                    <td>
                                    ...
                                    <td>
                                        @if($size->trashed())
                                            <nav class="nav table-options">
                                                <a class="nav-link" href="{{route('size.restore',$size->id)}}" title="Restore this size"><i data-feather="rotate-ccw"></i></a>
                                            </nav>
                                        @else
                                            <nav class="nav table-options">
                                                @if (auth()->user()->has_access_to_route('sizes.edit'))
                                                    <a class="nav-link" href="{{ route('sizes.edit',$size->id) }}" title="Edit Size"><i data-feather="edit"></i></a>
                                                @endif
                                                @if (auth()->user()->has_access_to_route('sizes.destroy'))
                                                    <a href="javascript:void(0)"
                                                        class="nav-link text-danger btn-delete"
                                                        data-id="{{ $size->id }}"
                                                        data-name="{{ $size->name }}"
                                                        data-url="{{ route('sizes.destroy', $size->id) }}"
                                                        title="Delete Size">
                                                            <i data-feather="trash"></i>
                                                    </a>
                                                @endif
                                                @if (auth()->user()->has_access_to_route('sizes.change-status'))
                                                    <a class="nav-link" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                        <i data-feather="settings"></i>
                                                    </a>
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        @if($size->status == 'PUBLISHED')
                                                            <a class="dropdown-item" href="{{route('size.change-status',[$size->id,'PRIVATE'])}}" > Private</a>
                                                        @else
                                                            <a class="dropdown-item" href="{{route('size.change-status',[$size->id,'PUBLISHED'])}}"> Publish</a>
                                                        @endif
                                                    </div>
                                                @endif
                                            </nav>
                                ...
                                @empty
                                <tr>
                                    <th colspan="6" style="text-align: center;"> <p class="text-danger">No sizes found.</p></th>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            <div class="col-md-6">
                <div class="mg-t-5">
                    @if ($sizes->firstItem() == null)
                        <p class="tx-gray-400 tx-12 d-inline">{{__('common.showing_zero_items')}}</p>
                    @else
                        <p class="tx-gray-400 tx-12 d-inline">Showing {{ $sizes->firstItem() }} to {{ $sizes->lastItem() }} of {{ $sizes->total() }} items</p>
                    @endif
                </div>
            </div>
            <div class="col-md-6">
                <div class="text-md-right float-md-right mg-t-5">
                    <div>
                        {{ $sizes->appends((array) $filter)->links() }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal effect-scale" id="prompt-update-status" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    You are about to <span id="sizeStatus"></span> this item. Do you want to continue?
                </div>
                <div class="modal-footer">
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('sizes.index') }}";
...
@section('customjs')
    <script>
        ...
        function change_status(status){
            ...
            if(parseInt(counter) < 1){
                ...
                if(parseInt(counter)>1){ // ask for confirmation when multiple pages was selected
                    $('#sizeStatus').html(status)
                    $('#prompt-update-status').modal('show');

                    $('#btnUpdateStatus').on('click', function() {
                        post_form("{{route('size.multiple.change.status')}}",status,selected_videos);
                    });
                }
                else{
                    post_form("{{route('size.multiple.change.status')}}",status,selected_videos);
        ...
        function post_form(url,status,size){
        ...
        function delete_size(){
            ...
            if(parseInt(counter) < 1){
                ...
                $('#btnDeleteMultiple').on('click', function() {
                    post_form("{{route('size.multiple.delete')}}",'',selected_videos);
                });
            }
        }

        function delete_one_size(id,page){
            $('#prompt-delete').modal('show');
            // $('#btnDelete').on('click', function() {
            //     // post_form("{{route('size.single.delete')}}",'',id);
            // });
        }
