...
<div class="container">
    ...
    <form action="" method="get">
        ...
        <table>
            ...
            <tr>
                ...
                <td><a href="{{ route('admin.report.sales-per-branch') }}" class="btn btn-success" style="margin:0px 0px 0px 5px;">Reset</a></td>
            </tr>
        </table>
    </form>
    <br>
    ...
    <table id="example" class="display nowrap" style="width:100%;font: normal 13px/150% Arial, sans-serif, Helvetica;">
        ...
        <tbody>
            @foreach($rs as $r)
                @php
                    $order_volume = \App\EcommerceModel\SalesHeader::branch_order_volume($r->order_source,$startDate,$endDate);
                @endphp
                <tr>
                    <td>{{ $r->order_source }}</td>
                    <td>{{ number_format($r->total_order,0) }}</td>
                    <td>{{ $order_volume }}</td>
                   <td class="text-right">{{ number_format($r->total_revenue,2) }}</td> 
                </tr>
            @endforeach
        </tbody>
    </table>
    ...
    $(document).ready(function() {
        $('#example').DataTable( {
            ...
            buttons: [
                ...
                {
                    extend: 'csv',
                    filename: 'BranchReport',
                    title: 'Branch Report ({{$date_display}})',
                    exportOptions: {
                        columns: ':visible'
                    }
                },
