                        ...
                        <table class="table mg-b-0 table-light table-hover table-striped">
                            ...
                            @forelse($sales as $sale)
                                ...
                                <tr style="height:30px; @if($sale['trashed']) background-color:#FFA07A; @endif {{ $sale['isConfirm'] == 1 ? 'disabled' : '' }}">
                                    ...
                                    <td>
                                        @php
                                        $saleHeader = \App\EcommerceModel\SalesHeader::with('items', 'payments')->find($sale['id']);
                                        @endphp
                                        @if ($saleHeader?->items?->count() > 0)
                                        {{ number_format(($saleHeader?->items?->sum('net_amount') + ($saleHeader->delivery_fee_amount ?? 0)) - $saleHeader->payments->where('is_discount', 1)->where('status', 'PAID')->sum('amount'), 2) }}
                                        @endif
                                    </td>
        ...
        function change_delivery_status(id,is_allowed_delivered, delivery_type){
            ...
            $.ajax({
                ...
                success: function( response ) {
                    ...
                    if (response.status.images && response.status.status != 'In Transit') {
