...
@section('content')
    ...
    @php
        /**
         * For multiple address / sub orders:
         * - $sales = current viewed order/sub order
         * - $mainSales = parent/main order passed from controller
         * Products/details stay from $salesDetails, but discounts/payments/totals use $displaySales.
         */
        $displaySales = $mainSales ?? $sales;

        $displayCouponRows = isset($couponRows)
            ? $couponRows
            : ($displaySales->couponUsed ?? collect());

        $couponDiscountTotal = collect($displayCouponRows)->sum('discount_used');

        $displayDiscount = max(
            (float) ($displaySales->discount_amount ?? 0),
            (float) $couponDiscountTotal
        );

        $displayDeliveryFee = (float) ($displaySales->delivery_fee_amount ?? 0);
    @endphp
                                                ...
                                                <table class="table table-invoice bd-b">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        @forelse($products as $product)
                                                            ...
                                                            // Use saved price first, not product table price
                                                            $savedPrice = data_get($product, 'price');
                                                            $hasSavedPrice = !is_null($savedPrice);

                                                            // Check if this product was saved as free in sales details
                                                            $matchingFreeDetail = isset($salesDetails)
                                                                ? $salesDetails->first(function ($detail) use ($product) {
                                                                    return (int) ($detail->product_id ?? 0) === (int) ($product->product_id ?? 0)
                                                                        && (float) ($detail->price ?? 0) <= 0;
                                                                })
                                                                : null;

                                                            $isFreeProduct = (bool) data_get($product, 'is_free_product', false)
                                                                || !empty(data_get($product, 'coupon_code'))
                                                                || !empty($matchingFreeDetail)
                                                                || ($hasSavedPrice && (float) $savedPrice <= 0);

                                                            $unitPrice = $isFreeProduct
                                                                ? 0
                                                                : (float) ($savedPrice ?? $prod->price ?? 0);

                                                            $paellaUnitPrice = $isFreeProduct
                                                                ? 0
                                                                : (!empty($product->paella) ? (float) ($prod->paella_price ?? 0) : 0);

                                                            $lineTotal = ($unitPrice * $qty) + ($paellaUnitPrice * $qty);
                                                        ...
                                                        @endphp
                                                            <tr>
                                                                ...
                                                                <td class="tx-right">
                                                                   ₱{{ number_format($paellaUnitPrice, 2) }}
                                                                </td>
                                                                <td class="tx-right">
                            ...
                            <tbody>
                                ...
                                @if($displayDeliveryFee > 0)
                                    <tr>
                                        ...
                                        <td class="tx-right">₱{{ number_format($displayDeliveryFee, 2) }}</td>
                                    </tr>
                                @endif
                                ...
                                @if($salesDetails->sum('gross_amount') > 0)
                                    <tr>
                                        ...
                                        <td class="tx-right">₱{{ number_format($salesDetails->sum('gross_amount') + $displayDeliveryFee, 2) }}</td>
                                    </tr>
                                @endif

                                @if($displayDiscount > 0 && (!isset($displayCouponRows) || count($displayCouponRows) == 0))
                                    <tr>
                                        <td class="tx-left" colspan="8">Discount</td>
                                        <td class="tx-right text-danger">-₱{{ number_format($displayDiscount, 2) }}</td>
                                    </tr>
                                @endif
                                ...
                                @forelse($gc as $g)
                                    <tr style="font-weight:bold;">
                                        ...
                                        <td class="tx-right text-danger">-₱{{ number_format($g->amount, 2) }}</td>
                                    </tr>
                                @empty
                                @endforelse
                        ...
                        <tbody>
                            ...
                            @forelse($salesPayments as $payment)
                                ...
                                <tr>
                                    ...
                                    <td class="tx-right {{ $payment->is_discount == 1 ? 'text-danger' : '' }}">
                            ...
                            @if($salesPayments->where('status', 'PAID')->sum('amount') > 0)
                                <tr style="font-weight:bold;">
                                    <td class="tx-left" colspan="4">Total</td>
                                    <td class="tx-right">
                                        ₱{{ number_format($salesPayments->where('status', '!=', 'CANCELLED')->where('is_discount', 0)->sum('amount'), 2) }}
                                    </td>
                                </tr>
                            @endif
                            ...
                            @php
                                $total_balance = (
                                    ...
                                    + $displayDeliveryFee
                                    - $displayDiscount
                                ) - $salesPayments->where('status', 'PAID')->sum('amount');
                            @endphp

                            @if($total_balance > 0 && $salesPayments->where('status', 'PAID')->sum('amount') > 0)
