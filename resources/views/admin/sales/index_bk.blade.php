...
@section('content')
                                                    ...
                                                    <div class="nav-item dropdown">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-right">
                                                            ...
                                                            @if($dateneeded > date('Y-m-d H:i:s'))
                                                                    <a class="dropdown-item"  href="javascript:void(0);" onclick="update_items({{$sale->id}});" title="Confirm Order" >Update Items</a>
                                                                    
                                                            
                                                                @if(auth()->user()->role_id == 2 || auth()->user()->role_id == 1)
                                                                    <!-- onclick="update_dateneeded({{$sale->id}},'{{ $dateneeded }}','{{$sale->delivery_type}}','{{$locationed}}','{{$sale->instruction}}','{{$sale->customer_delivery_adress}}');"  -->
                                                                    <a class="dropdown-item"  href="javascript:void(0);"                
                                                                    onclick="update_dateneeded_new({{$sale->id}});"
                                                                    title="Confirm Order" >Update Sales Details</a>
                                                                @endif
    ...
    <div class="modal effect-scale" id="prompt-update-dateneeded" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <form method="post" action="{{route('update_dateneeded')}}">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalCenterTitle">Update Sales Details</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        @csrf
                        <h5 id="update_dateneeded_old"></h5>
                        <input type="hidden" name="update_dateneeded_id" id="update_dateneeded_id">
                        <input type="hidden" name="update_dateneeded_deliverytype" id="update_dateneeded_deliverytype"> 
                        <div class="form-group">
                            <label class="d-block">Location *</label>

                            <select class="form-control" style="display:none;" name="update_dateneeded_sp" id="update_dateneeded_sp">
                            ...
                            <select class="form-control" style="display:none;" name="update_dateneeded_d2d" id="update_dateneeded_d2d">
                                <option value="">Select new location</option>
                        ...
                        <div class="form-group">
                            ...
                            <input required type="date" class="form-control" name="update_dateneeded_date" id="update_dateneeded_date">
                        </div>
                        <div class="form-group">
                            <label class="d-block">Time *</label>
                            <select required class="form-control" name="update_dateneeded_time" id="update_dateneeded_time">                                       
                                        <option value="">Select</option>
                        ...
                        <div class="form-group">
                            <label class="d-block">Delivery Address</label>
                            <textarea name="new_delivery_address" class="form-control" id="new_delivery_address" cols="30" rows="4"></textarea>
                        </div>
                        <div class="form-group">
                            <label class="d-block">Instruction</label>
                            <textarea name="new_instruction" class="form-control" id="new_instruction" cols="30" rows="4"></textarea>
                        </div>
    ...
    <div class="modal effect-scale" id="prompt-update-items" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <form action="{{route('sales-transaction.update_items')}}" method="post">
                @csrf
                <input type="hidden" name="ui_sales_id" id="ui_sales_id">
                <input type="hidden" name="ui_total_new" id="ui_total_new" value="0">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalCenterTitle">Update Items</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body" id="update_items_tbl">
...
@section('customjs')
    <script>
        ...
        function update_items(x){
            $('#ui_sales_id').val(x);
            $.ajax({
                type: "GET",
                url: "{{ route('sales-transaction.edit_items') }}",
                data: { id : x },
                success: function( response ) {
                    $('#update_items_tbl').html(response);
                    $('#prompt-update-items').modal('show');
                }
            });            
        }
        ...
        function update_dateneeded(x,y,tayp,loc,instr,deladd){    
           
            $('#update_dateneeded_old').html('Old date: '+y+' <br>Old location: '+loc+' ('+tayp+')');
            $('#update_dateneeded_id').val(x);
            $('#new_instruction').val(instr);
            $('#new_delivery_address').val(deladd);
            $('#update_dateneeded_deliverytype').val(tayp);
            if(tayp == 'Door to door delivery'){
                $('#update_dateneeded_sp').hide();
                $('#update_dateneeded_d2d').show();
            }
            else if(tayp == 'Store Pickup'){
                $('#update_dateneeded_sp').show();
                $('#update_dateneeded_d2d').hide();
            }
            $('#prompt-update-dateneeded').modal('show');
        }

        function update_dateneeded_new(x){    
            $.ajax({
                type: "GET",
                url: "{{ route('prepare_dateneeded') }}",
                data: { id : x },
                success: function( response ) {

                    $('#update_dateneeded_old').html('Old date: '+response.dateneeded+' <br>Old location: '+response.location+' ('+response.delivery_type+')');
                    $('#update_dateneeded_id').val(response.id);
                    $('#new_instruction').val(response.instruction);
                    $('#new_delivery_address').val(response.delivery_address);
                    $('#update_dateneeded_deliverytype').val(response.delivery_type);
                    $('#update_dateneeded_date').val(response.date_only);
                    $('#update_dateneeded_time').val(response.time_only);
                    if(response.delivery_type == 'Door to door delivery'){
                        $('#update_dateneeded_d2d').val(response.location);
                        $('#update_dateneeded_sp').hide();
                        $('#update_dateneeded_d2d').show();
                    }
                    else if(response.delivery_type == 'Store Pickup'){
                        $('#update_dateneeded_sp').val(response.location);
                        $('#update_dateneeded_sp').show();
                        $('#update_dateneeded_d2d').hide();
                    }
                    $('#prompt-update-dateneeded').modal('show');
