...
<div class="container pd-x-0">
	...
	@endif
		<form action="{{ route('popup-message.update', $popup_message->id) }}" method="POST" id="edit_popup_message_form" autocomplete="off">
		@csrf
		@method('PUT')
		<div class="row row-sm">
			<div class="col-lg-6">
				<div class="form-group">
					...
					<input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $popup_message->title) }}">
				...
				<div class="form-group">
					...
					<textarea name="message" id="editor1" rows="10" cols="80">
							{{ old('message', $popup_message->message) }}
					</textarea>
					<x-error-message inputName="message" />
				</div>
				<div class="form-group">
					<label class="d-block">Button text *</label>
					<input type="text" name="button_text" class="form-control @error('button_text') is-invalid @enderror" value="{{ old('button_text', $popup_message->button_text) }}">
					<x-error-message inputName="button_text" />
				</div>
				<div class="form-group">
					<label class="d-block">Button text URL*</label>
					<input type="text" name="button_text_url" class="form-control @error('button_text_url') is-invalid @enderror" value="{{ old('button_text_url', $popup_message->button_text_url) }}">
					<x-error-message inputName="button_text_url" />
				</div>
				<div class="form-group">
					<label class="d-block">Close Button text *</label>
					<input type="text" name="close_button_text" class="form-control @error('close_button_text') is-invalid @enderror" value="{{ old('close_button_text', $popup_message->close_button_text) }}">
				...
				<div class="form-group">
					...
					<input
						...
						value="{{ old('start_at', optional($popup_message->start_at)->format('Y-m-d\TH:i')) }}"
					>
				...
				<div class="form-group">
					...
					<input
						...
						value="{{ old('expire_at', optional($popup_message->expire_at)->format('Y-m-d\TH:i')) }}"
					>
					<p class="tx-11 mg-t-4">
				...
				<div class="form-group">
					...
					<input type="text" name="start_to_show" class="form-control @error('start_to_show') is-invalid @enderror" value="{{ old('start_to_show', $popup_message->start_to_show) }}">
			...
			<div class="col-lg-12">
				<div class="form-group">
					...
					<div class="custom-control custom-switch">
						<input type="checkbox" class="custom-control-input" id="enableSwitch1" name="is_active" {{ old('is_active', $popup_message->is_active == 1) ? 'checked' : '' }}>
						<label class="custom-control-label" for="enableSwitch1" id="label_status">@if(old('is_active', $popup_message->is_active == 1)) Active @else Inactive @endif</label>
					</div>
				</div>
			</div>
...
@section('customjs')
<script>
	$("#edit_popup_message_form").submit(function(e){
