            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-style1 mg-b-5">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Delivery Rates</li>
                    </ol>
                </nav>
                <h4 class="mg-b-0 tx-spacing--1">Delivery Rates Manager</h4>
            </div>
        </div>
                                    ...
                                    <form id="filterForm" class="pd-20">
                                        <div class="form-group">
                                            ...
                                            <div class="custom-control custom-radio">
                                                <input type="radio" id="orderBy1" name="orderBy" class="custom-control-input" value="region" @if ($filter->orderBy == 'region') checked @endif>
                                                <label class="custom-control-label" for="orderBy1">Region</label>
                                            </div>
                                            <div class="custom-control custom-radio">
                                                <input type="radio" id="orderBy2" name="orderBy" class="custom-control-input" value="province" @if ($filter->orderBy == 'province') checked @endif>
                                                <label class="custom-control-label" for="orderBy2">Province</label>
                                            </div>
                                        </div>
                                        <div class="form-group">
                        ...
                        <table class="table mg-b-0 table-light table-hover">
                            ...
                            @forelse($address as $add)
                                <tr>
                                    ...
                                    <td> <strong @if($add->trashed()) style="text-decoration:line-through;" @endif> {{$add->region }}</strong></td>
                                    <td>{{ $add->province }}</td>
                                    <td>{{ $add->municipality }}</td>
                                    <td>{{ $add->brgy }}</td>
                                    <td>{{ $add->zip }}</td>
                                    <td>{{ $add->sla }}</td>
                                    <td align="right">{{ number_format($add->rate,2) }}</td>
                                    <td align="right">{{ number_format($add->excess_fee,2) }}</td>
                                    <td>
                                        <nav class="nav table-options">
                                            @if($add->trashed())
                                                <nav class="nav table-options">
                                                    <a class="nav-link" href="{{route('deliveryrate.restore',$add->id)}}" title="Restore this product"><i data-feather="rotate-ccw"></i></a>
                                                </nav>
                                            @else
                                                <a class="nav-link" href="{{ route('deliveryrate.edit',$add->id) }}" title="Edit Product"><i data-feather="edit"></i></a>
                                                <a class="nav-link" href="javascript:void(0)" onclick="delete_one_category({{$add->id}},'{{$add->region}}')" title="Delete Product"><i data-feather="trash"></i></a>
                                            @endif
                                        </nav>
                                    </td>
                                </tr>
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('deliveryrate.index') }}";
    ...
    <script>
        ...
        function delete_category(){
            ...
            if(parseInt(counter) < 1){
                ...
                $('#btnDeleteMultiple').on('click', function() {
                    post_form("{{route('deliveryrate.multiple.delete')}}",'',selected_products);
                });
            }
        }
        ...
        function delete_one_category(id){
            ...
            $('#btnDelete').on('click', function() {
                // console.log(id);
                post_form("{{route('deliveryrate.single.delete')}}",'',id);
            });
        }
