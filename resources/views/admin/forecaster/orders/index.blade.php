    ...
    <div class="container pd-x-0">
        <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
            <div>
                ...
                <h4 class="mg-b-0 tx-spacing--1">Order Manager</h4>
            </div>
        </div>
...
{{--                        </div>--}}
                        ...
                        <table class="table mg-b-0 table-light table-hover">
                            ...
                            @forelse($orders as $order)
                                <tr>
                                    ...
                                    <td> <strong @if($order->trashed()) style="text-decoration:line-through;" @endif> {{$order->order_number }}</strong></td>
                                    ...
                                    <td>
                                        <nav class="nav table-options">
                                            @if($order->trashed())
                                                <nav class="nav table-options">
                                                    <a class="nav-link" href="{{route('production-branches.restore',$order->id)}}" title="Restore this product"><i data-feather="rotate-ccw"></i></a>
                                                </nav>
                                            @else
                                                <a class="nav-link" href="{{ route('production-branches.edit',$order->id) }}" title="Edit Product"><i data-feather="edit"></i></a>

                                                <a class="nav-link" href="javascript:void(0)" onclick="delete_one_category({{$order->id}},'{{$order->order_number}}')" title="Delete Product"><i data-feather="trash"></i></a>
                                            @endif
                                        </nav>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <th colspan="7" style="text-align: center;"> <p class="text-danger">No Order found.</p></th>
                                </tr>
                            @endforelse
                            </tbody>
                        </table>
            ...
            <div class="col-md-6">
                <div class="mg-t-5">
                    @if ($orders->firstItem() == null)
                        <p class="tx-gray-400 tx-12 d-inline">{{__('common.showing_zero_items')}}</p>
                    @else
                        <p class="tx-gray-400 tx-12 d-inline">Showing {{ $orders->firstItem() }} to {{ $orders->lastItem() }} of {{ $orders->total() }} items</p>
                    @endif
                </div>
            </div>
            <div class="col-md-6">
                <div class="text-md-right float-md-right mg-t-5">
                    <div>
                        {{ $orders->appends((array) $filter)->links() }}
                    </div>
                </div>
            </div>
        </div>
