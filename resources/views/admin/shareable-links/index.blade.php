    ...
    <div class="container pd-x-0">
        <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
            <div>
                ...
                <h4 class="mg-b-0 tx-spacing--1">Social Media Shareable Links Manager</h4>
            </div>
        </div>
        ...
        <div class="row row-sm">
            ...
            <div class="col-md-12">
                <div class="filter-buttons">
                    <div class="d-md-flex bd-highlight">
                        ...
                        <div class="ml-auto bd-highlight mg-t-15">
                        ...
                        <div class="mg-t-10">
                            @if (auth()->user()->has_access_to_route('shareable-links.create'))
                                <a class="btn btn-primary btn-sm mg-b-10" href="{{route('shareable-links.create')}}">Create a Link</a>
                            @endif
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table mg-b-0 table-light table-hover">
                            ...
                            @forelse($links as $link)
                                <tr>
                                    <th>
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input cb" id="cb{{ $link->id }}">
                                            <label class="custom-control-label" for="cb{{ $link->id }}"></label>
                                        </div>
                                    </th>
                                    <td> <strong @if($link->trashed()) style="text-decoration:line-through;" @endif> {{$link->name }}</strong></td>
                                    <td>{{ $link->soc_media }}</td>
                                    <td><p id="p{{$link->id}}">{{ $link->url }}</p></td>
                                    <td>{{ Setting::date_for_listing($link->updated_at) }}</td>
                                    <td>
                                        <input type="hidden" id="link{{$link->id}}" value="{{ $link->url }}">
                                        <nav class="nav table-options">
                                            @if($link->trashed())
                                                @if (auth()->user()->has_access_to_route('shareable-link.restore'))
                                                    <a class="nav-link" href="{{route('shareable-link.restore',$link->id)}}" title="Restore this link"><i data-feather="rotate-ccw"></i></a>
                                                @endif
                                            @else
                                                <a class="nav-link" href="javascript:;" title="Copy Link" onclick="copyToClipboard('#p{{$link->id}}')"><i data-feather="copy"></i></a>

                                                @if (auth()->user()->has_access_to_route('branch.edit'))
                                                    <a class="nav-link" href="{{ route('shareable-links.edit',$link->id) }}" title="Edit Link"><i data-feather="edit"></i></a>
                                                @endif
                                                @if (auth()->user()->has_access_to_route('branch.single.delete'))
                                                    <a class="nav-link" href="javascript:void(0)" onclick="delete_one_category({{$link->id}},'{{$link->name}}')" title="Delete Link"><i data-feather="trash"></i></a>
                                                @endif
                                            @endif
                                        </nav>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <th colspan="7" style="text-align: center;"> <p class="text-danger">No shareable links found.</p></th>
                                </tr>
                            @endforelse
                            </tbody>
                        </table>
            ...
            <div class="col-md-6">
                <div class="mg-t-5">
                    @if ($links->firstItem() == null)
                        <p class="tx-gray-400 tx-12 d-inline">{{__('common.showing_zero_items')}}</p>
                    @else
                        <p class="tx-gray-400 tx-12 d-inline">Showing {{ $links->firstItem() }} to {{ $links->lastItem() }} of {{ $links->total() }} items</p>
                    @endif
                </div>
            </div>
            <div class="col-md-6">
                <div class="text-md-right float-md-right mg-t-5">
                    <div>
                        {{ $links->appends((array) $filter)->links() }}
                    </div>
                </div>
            </div>
        </div>
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('shareable-links.index') }}";
    ...
    <script>
        ...
        function delete_category(){
            ...
            $(".cb:checked").each(function(){
                counter++;
                fid = $(this).attr('id');
                selected_links += fid.substring(2, fid.length)+'|';
            });
            ...
            if(parseInt(counter) < 1){
                ...
                $('#btnDeleteMultiple').on('click', function() {
                    post_form("{{route('shareable-link.multiple.delete')}}",selected_links);
                });
            }
        }
        ...
        function delete_one_category(id){
            ...
            $('#btnDelete').on('click', function() {
                post_form("{{route('shareable-link.single.delete')}}",id);
            });
        }
