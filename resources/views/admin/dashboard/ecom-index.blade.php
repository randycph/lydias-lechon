    ...
    <div class="row row-sm">
                ...
                <div class="d-md-flex align-items-center justify-content-between">
                    <div class="media align-sm-items-center">
                        <div class="media-body">
                            ...
                            <a href="{{route('ecom-dashboard')}}?filter=sales&@if(isset($_GET['data_from']))data_from={{ $_GET['data_from'] }}@endif" class="btn btn-sm @if(!isset($_GET['filter']) || isset($_GET['filter']) && $_GET['filter'] == 'sales') btn-primary @else btn-white @endif btn-uppercase pd-x-15 mg-b-5">Sales</a>

                            <a href="{{route('ecom-dashboard')}}?filter=orders&@if(isset($_GET['data_from']))data_from={{ $_GET['data_from'] }}@endif" class="btn btn-sm @if(isset($_GET['filter']) && $_GET['filter'] == 'orders') btn-primary @else btn-white @endif btn-uppercase pd-x-15 mg-b-5 mg-t-5 mg-sm-t-0 mg-sm-l-5">Orders</a>

                            <a href="{{route('ecom-dashboard')}}?filter=volume&@if(isset($_GET['data_from']))data_from={{ $_GET['data_from'] }}@endif" class="btn btn-sm @if(isset($_GET['filter']) && $_GET['filter'] == 'volume') btn-primary @else btn-white @endif btn-uppercase pd-x-15 mg-b-5 mg-t-5 mg-sm-t-0 mg-sm-l-5">Volumes</a>
                        </div>
                    </div>
                ...
                <div class="card-body pd-y-25">
                    ...
                    <div class="col-12 tx-12 mg-t-40">
                        ...
                        @php
                            ...
                            $labels .= $l.'|';
                            $color = \App\EcommerceModel\SalesHeader::media_color($l);
                            $colors .= $color.'|';
                        @endphp
        ...
        <div class="col-lg-4 mg-t-10">
            <div class="card">
                <div class="card-body">
                        <div class="col-12">
                            <div class="chart-thirteen"><canvas id="chartBranch"></canvas></div>
                        </div>
                        <div class="col-12 tx-12 mg-t-40">
                            @php
                                $bcolors = '';
                                ...
                                $total_bsales = 0;
                            @endphp
                            @foreach($orders_per_branch as $order)

                            @php
                                $total_bsales += $order->filtered_value;
                                $bvalues .= $order->filtered_value.'|';
                                $bcolor = \App\EcommerceModel\SalesHeader::random_color();

                                $blabels .= $order->order_source.'|';
                                $bcolors .= $bcolor.'|';

                            @endphp
                            <div class="d-flex align-items-center mg-t-10">
                                <div class="wd-10 ht-10 rounded-circle pos-relative t--1" style="background-color:{{$bcolor}};"></div>
                                <span class="tx-medium mg-l-10">{{ ucwords($order->order_source) }}</span>
                                <span class="tx-rubik mg-l-auto">
                                    @isset($_GET['filter'])
                                        @if($_GET['filter'] == 'sales')
                            ...
                            @endforeach
                            <input type="hidden" id="per_branch_colors" value="{{ $bcolors }}">
                            <input type="hidden" id="per_branch_labels" value="{{ $blabels }}">
                            <input type="hidden" id="per_branch_revenues" value="{{ $bvalues }}">
                            <input type="hidden" id="total_branch_revenues" value="{{ $total_bsales }}">
                        </div>
                </div>
                <div class="card-footer text-center tx-13">
                    <a href="{{ route('admin.report.sales-per-branch') }}?startdate={{ \Carbon\Carbon::parse($startDate)->format('Y-m-d') }}&enddate={{ \Carbon\Carbon::parse($endDate)->format('Y-m-d') }}" target="_blank" class="link-03">View All Branches <i class="icon ion-md-arrow-forward mg-l-5"></i></a>
                </div>
            </div>
        </div>
                ...
                <div class="card-body">
                        ...
                        <div class="col-12 tx-12 mg-t-40">
                            ...
                            @php
                                ...
                                $cat_color = \App\EcommerceModel\SalesHeader::random_color();
...
<script>
    ...
    // line chart sales per month
        ...
        console.log(arr_yr2_sales);
        console.log(arr_yr3_sales);
    //



    // pie chart sales by branch
        var branch_colors = $('#per_branch_colors').val();
        var bcolors = branch_colors.split('|');
        var arr_bcolors = [];
        $.each(bcolors, function(key, value) {
            if(value != ""){
                arr_bcolors.push(value);
            }
        });

        var branch_labels = $('#per_branch_labels').val();
        var blabels = branch_labels.split('|');
        var arr_blabels = [];
        $.each(blabels, function(key, value) {
            if(value != ""){
                arr_blabels.push(value);
            }
        });

        var btotal_revenues = parseFloat($('#total_branch_revenues').val());
        var branch_revenues = $('#per_branch_revenues').val();
        var brevenues = branch_revenues.split('|');
        var arr_bpercent_revenues = [];
        $.each(brevenues, function(key, value) {
            if(value != ""){
                percnt = (parseFloat(value)/btotal_revenues)*100;
                arr_bpercent_revenues.push(percnt.toFixed(2));
            }
        });
        ...
        var datapie = {
            datasets: [{
                labels: arr_blabels,
                data: arr_bpercent_revenues,
                backgroundColor: arr_bcolors
            }]
        };
        ...
        var ctx9 = document.getElementById('chartBranch');
