    ...
    <div class="container pd-x-0">
        <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
            <div>
                ...
                <h4 class="mg-b-0 tx-spacing--1">Manage News & Blogs</h4>
            </div>
        </div>
                    ...
                    <div class="d-md-flex bd-highlight">
                        ...
                        <div class="mg-t-10">
                            @if(auth()->user()->has_access_to_route('news.create'))
                                ...
                                <a class="btn btn-primary btn-sm mg-b-20" href="{{ route('blogs.create') }}">Create a Blog</a>
                            @endif
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                @forelse($news as $new)
                                    <tr id="row{{$new->id}}" class="row_cb">
                                        ...
                                        <td>
                                            @if($new->external_link== '')<span class="badge badge-primary">News</span>@else <span class="badge badge-primary">Blogs</span>@endif
                                        </td>
                                            ...
                                            @else
                                                <nav class="nav table-options justify-content-end">
                                                    @if(auth()->user()->has_access_to_route('news.edit'))
                                                        @if(empty($new->external_link))
                                                            <a class="nav-link" href="{{ route('news.edit', $new->id) }}" title="Edit News"><i data-feather="edit"></i></a>
                                                        @else
                                                            <a class="nav-link" href="{{ route('blogs.edit', $new->id) }}" title="Edit Blog"><i data-feather="edit"></i></a>
                                                        @endif
                                                    @endif
                                                    @if(empty($new->external_link))
                                                        <a class="nav-link" target="_blank" href="{{route('news.front.show',$new->slug)}}" title="View Page"><i data-feather="eye"></i></a>
                                                    @else
                                                        <a class="nav-link" target="_blank" href="{{$new->external_link}}" title="View Page"><i data-feather="eye"></i></a>
                                                    @endif

                                                    @if(auth()->user()->has_access_to_route('news.edit') || auth()->user()->has_access_to_route('news.delete'))
