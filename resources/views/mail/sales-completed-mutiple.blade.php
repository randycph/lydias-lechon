...
| {{ $details->product->code }} | {{ $details->product_name }} @if($details->paella_price > 0) Boneless with Paella @endif | {{ number_format($details->qty, 0) }} | {{ number_format($details->paella_price + $details->price, 2) }} | {{ number_format($details->gross_amount, 2) }} |
@empty
| *No items found* |  |  |  |  |
@endforelse
