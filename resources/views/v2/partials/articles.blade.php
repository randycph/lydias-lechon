@foreach ($articles as $blog)
    <a href="{{ route('article', ['category' => $blog->category->slug, 'slug' => $blog->slug]) }}" class="rounded-lg">
        <img src="{{ $blog->thumbnail_url }}" alt="{{ $blog->name }}" class="w-full h-[188px] lg:h-[362px] object-cover rounded-t-lg hover:scale-105 transition duration-300 hover:opacity-80">
