                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="shopping-cart-wrapper">
                            ...
                            <form id="checkOutForm" action="{{route('kiosk.batch_update')}}" method="post">
                                @csrf
                                ...
                                @forelse($cart as $key => $order)
                                    @php
                                        ...
                                        if($order->product->production_item == 1){
                                            $lechon = 1;
                                        }
                                ...
                                @empty
                                    <div class="no-order-wrapper">
                                        <div class="alert alert-warning" role="alert">
                                            Your shopping cart is empty. Click <a href="{{ route('kiosk.menu') }}">here</a> to start shopping
                                        </div>
                                    </div>
                                @endforelse
                                <input type="hidden" name="customer_type" id="customer_type" value="0">
                                <input type="hidden" name="is_express" id="is_express" value="0">
                                <input type="hidden" name="has_lechon" value="{{ $counter }}">
                            </form>
                        </div>
                            ...
                            <div class="cart-summary">
                                ...
                                <table class="table">
                                    ...
                                    <tr>
                                        ...
                                        <td id="grandcoupon">&#8369; {{ number_format($coupon_amount,2) }}</td>
                                    </tr>
                                    <tr>
                                        <th>Total Amount</th>
                                ...
                                <a href="javascript:;" class="btn btn-secondary checkout-btn" id="checkout">Checkout</a>
                                @if($expCounter == $cart->count())
                                <a href="javascript:;" class="btn btn-secondary checkout-btn" id="express_checkout">Express Checkout</a>
                                @endif
                                <a class="btn btn-primary add-btn" href="{{route('kiosk.menu')}}" @if($expCounter > 0) style="margin-top:10px;" @endif>Add more products</a>
                            </div>
                        </div>
                    </div>
                @endif
...
@section('jsscript')
    ...
    <script>
        ...
        function compute_grand_total(){
            ...
            totalAmount = summary_sub_price-parseFloat($('#coupon_amount').val());
            ...
            $('#grandtotal').html('&#8369; '+addCommas(totalAmount.toFixed(2)));
