...
@section('content')
    <section>       
        <div class="content-wrapper">
            ...
            <div class="container">
                <div class="row">
                    <input type="hidden" id="minimum_order_amt" value="@if (session('delivery_option') == 'delivery') {{Setting::info()->minimum_order}} @else {{Setting::info()->minimum_order_pickup}} @endif">
                    <div class="col-md-12">
                            ...
                            <div class="cart-summary">
                                ...
                                <table class="table">
                                    ...
                                    <tr>
                                        ...
                                        <input type="hidden" id="hidden_total" value="0.00">
                                    </tr>
                                    </tbody>
                                </table>
                                @guest
    ...
    <div class="modal effect-scale" id="insufficient-order-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalCenterTitle">Insufficient Total Order</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <p>A minimum of Php @if (session('delivery_option') == 'delivery') {{number_format(Setting::info()->minimum_order,2)}} @else {{number_format(Setting::info()->minimum_order_pickup,2)}} @endif is required in order to checkout your order.</p>
                </div>
                <div class="modal-footer">
                    <a href="{{route('product.front.show_forsale')}}" class="btn btn-sm btn-info">Add more products</a>
                    <button type="button" class="btn btn-sm btn-secondary" data-dismiss="modal">Close</button>
                </div>
            </div>
        </div>
...
@section('jsscript')
    ...
    <script>
        ...
        function compute(x){
            ...
            $('#hidden_total').val(summary_sub_price);
        ...
        $(document).ready(function(){
            ...
            $('.checkout-btn').on('click', function () {
                ...
                var tots = parseFloat($('#hidden_total').val());
                var min_amt = parseFloat($('#minimum_order_amt').val());

                if(tots < min_amt){
                    $('#insufficient-order-modal').modal('show');
                    return false;
                }
